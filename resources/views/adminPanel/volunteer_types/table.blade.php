        ...
        <div class="col-lg-12 col-xl-12">
            <div class="row align-items-center">
                <div class="col-md-12 my-md-0">
                    {!! Form::open(['route' => 'adminPanel.volunteerTypes.index', 'method'=>'GET']) !!}
                        <div class="row">
                            ...
                            <div class="form-group col-sm-4">
                                ...
                                <a href="{{ route('adminPanel.volunteerTypes.index') }}" class="btn btn-light-danger font-weight-bold">@lang('crud.reset')</a>
                            ...
                            <div class="form-group col-sm-12">
                                @error('export_rows')
                                    <h1 class="text-danger">
                                        @lang('lang.select_to_export', ['model' => __('models/volunteerTypes.plural')])
                                    </h1>
                                @enderror
                            </div>
                        </div>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    <thead>
        <tr>
            ...
            <th>@lang('models/volunteerTypes.fields.name')</th>
            <th>@lang('crud.action')</th>
        </tr>
    </thead>
    <tbody>
        {!! Form::open(['route' => ['adminPanel.data.export', 'volunteerTypes'], 'id' => 'export-data']) !!}
        @foreach($volunteerTypes as $volunteerType)
        <tr>
            <td>
                <label class="checkbox">
                    <input type="checkbox" class="check_inputs inputs-permmission control-input" value="{{ $volunteerType->id }}" name="export_rows[]">
                    <span></span>
                </label>
            </td>
            <td>{{ $volunteerType->name }}</td>
            <td>
                ...
                <div class='btn btn-sm-group'>
                    ...
                    @endcan
                    @can('volunteerTypes destroy')
                    {{-- {!! Form::button('<i class="fa fa-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-sm btn-shadow mx-1 btn-transparent-danger', 'onclick' => 'return confirm("'.__('crud.are_you_sure').'")']) !!} --}}
                    <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger" data-toggle="modal" data-target="#country-{{$volunteerType->id}}-modal">
...
<!-- Modal -->
<div class="modal fade" id="country-{{$volunteerType->id}}-modal" tabindex="-1" role="dialog" aria-labelledby="country-{{$volunteerType->id}}"
    aria-hidden="true">
