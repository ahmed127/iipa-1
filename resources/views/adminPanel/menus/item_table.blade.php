...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    ...
    <tbody>
        ...
        <tr>
            <td>
                <label class="checkbox">
                    <input type="checkbox" class="check_inputs inputs-permmission control-input" value="{{ $item->id }}" name="export_rows[]">
                    <span></span>
                </label>
            </td>
            <td>{{ $item->name??'' }}</td>
            ...
            <td><a href="{{ route('adminPanel.menus.item_index', $menu->id)."?parent_id=$item->id" }}" class="btn btn-primary">{{ $item->routes_count }}</a></td>
            <td>
                {{-- <div class='btn btn-sm-group'>
                    @can('menus view')
                    <a href="{{ route('adminPanel.menus.show', [$item->id]) }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                        <i class="fa fa-eye"></i>
                    </a>
                    @endcan
                    @can('menus edit')
                    <a href="{{ route('adminPanel.menus.edit', [$item->id]) . "?languages=en" }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-primary'>
                        <i class="fa fa-edit"></i>
                    </a>
                    @endcan
                    @can('menus destroy')
                    <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger" data-toggle="modal" data-target="#country-{{$item->id}}-modal">
...
{{-- @can('menus destroy')
    ...
    <!-- Modal -->
    <div class="modal fade" id="country-{{$item->id}}-modal" tabindex="-1" role="dialog" aria-labelledby="country-{{$item->id}}"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    {!! Form::open(['route' => ['adminPanel.menus.destroy', $item->id], 'method' => 'delete']) !!}
