                ...
                <div class="col-md-12 my-md-0">
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <a href="{{ route('adminPanel.directors.index') }}"
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    <thead>
        <tr>
            ...
            <th>@lang('models/directors.fields.nickname')</th>
                ...
                <td>
                    <label class="checkbox">
                        <input type="checkbox" class="check_inputs inputs-permmission control-input"
                            value="{{ $director->id }}" name="export_rows[]">
                        <span></span>
                    </label>
                </td>
                <td> <img src="{{ $director->photo }}" alt="{{ $director->name }}" width="100"></td>
                <td>{{ $director->name }}</td>
                <td>{{ $director->nickname }}</td>
                <td>{{ $director->job_title }}</td>
                <td>
                    {{-- {!! Form::open(['route' => ['adminPanel.directors.destroy', $director->id], 'method' => 'delete']) !!} --}}
                    <div class='btn btn-sm-group'>
                        @can('directors view')
                            <a href="{{ route('adminPanel.directors.show', [$director->id]) }}"
                                class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                                <i class="fa fa-eye"></i>
                            </a>
                        @endcan
                        @can('directors edit')
                            <a href="{{ route('adminPanel.directors.edit', [$director->id]) . '?languages=en' }}"
                        ...
                        @can('directors destroy')
                            ...
                            <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger"
                                data-toggle="modal" data-target="#country-{{ $director->id }}-modal">
...
@can('directors destroy')
    @foreach ($directors as $director)
        <!-- Modal -->
        <div class="modal fade" id="country-{{ $director->id }}-modal" tabindex="-1" role="dialog"
            aria-labelledby="country-{{ $director->id }}" aria-hidden="true">
