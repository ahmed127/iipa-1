        ...
        <div class="col-lg-12 col-xl-12">
            <div class="row align-items-center">
                <div class="col-md-12 my-md-0">
                    {!! Form::open(['route' => 'adminPanel.generals.index', 'method' => 'GET']) !!}
                    <div class="row">
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <a href="{{ route('adminPanel.generals.index') }}"
                        ...
                        <div class="form-group col-sm-12">
                            @error('export_rows')
                                <h1 class="text-danger">
                                    @lang('lang.select_to_export', ['model' => __('models/generals.plural')])
                                </h1>
                            @enderror
                        </div>
                    </div>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    <thead>
        <tr>
            ...
            <th>@lang('models/generals.fields.photo')</th>
            <th>@lang('models/generals.fields.name')</th>
            <th>@lang('models/generals.fields.nickname')</th>
            <th>@lang('models/generals.fields.job_title')</th>
            <th>@lang('crud.action')</th>
        </tr>
    </thead>
    <tbody>
        {!! Form::open(['route' => ['adminPanel.data.export', 'generals'], 'id' => 'export-data']) !!}
        @foreach ($generals as $general)
            <tr>
                <td>
                    <label class="checkbox">
                        <input type="checkbox" class="check_inputs inputs-permmission control-input"
                            value="{{ $general->id }}" name="export_rows[]">
                        <span></span>
                    </label>
                </td>
                <td> <img src="{{ $general->photo }}" alt="{{ $general->name }}" width="100"></td>
                <td>{{ $general->name }}</td>
                <td>{{ $general->nickname }}</td>
                <td>{{ $general->job_title }}</td>
                <td>
                    {{-- {!! Form::open(['route' => ['adminPanel.generals.destroy', $general->id], 'method' => 'delete']) !!} --}}
                    <div class='btn btn-sm-group'>
                        @can('generals view')
                            <a href="{{ route('adminPanel.generals.show', [$general->id]) }}"
                                class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                                <i class="fa fa-eye"></i>
                            </a>
                        @endcan
                        @can('generals edit')
                            <a href="{{ route('adminPanel.generals.edit', [$general->id]) . '?languages=en' }}"
                                class='btn btn-sm btn-shadow mx-1 btn-transparent-primary'>
                                <i class="fa fa-edit"></i>
                            </a>
                        @endcan
                        @can('generals destroy')
                            {{-- {!! Form::button('<i class="fa fa-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-sm btn-shadow mx-1 btn-transparent-danger', 'onclick' => 'return confirm("'.__('crud.are_you_sure').'")']) !!} --}}
                            <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger"
                                data-toggle="modal" data-target="#country-{{ $general->id }}-modal">
...
@can('generals destroy')
    @foreach ($generals as $general)
        <!-- Modal -->
        <div class="modal fade" id="country-{{ $general->id }}-modal" tabindex="-1" role="dialog"
            aria-labelledby="country-{{ $general->id }}" aria-hidden="true">
