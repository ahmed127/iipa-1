                    ...
                    {!! Form::open(['route' => 'adminPanel.cooperativeTrainings.index', 'method'=>'GET']) !!}
                        <div class="row">
                            ...
                            <div class="form-group col-sm-4">
                                ...
                                <a href="{{ route('adminPanel.cooperativeTrainings.index') }}" class="btn btn-light-danger font-weight-bold">@lang('crud.reset')</a>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    ...
    <tbody>
        ...
        <tr>
            <td>
                <label class="checkbox">
                    <input type="checkbox" class="check_inputs inputs-permmission control-input" value="{{ $cooperativeTraining->id }}" name="export_rows[]">
                    <span></span>
                </label>
            </td>
            ...
            <td>{{ $cooperativeTraining->attachment_letter }}</td>
            <td>
                ...
                <div class='btn btn-sm-group'>
                    ...
                    <a href="{{ route('adminPanel.cooperativeTrainings.show', [$cooperativeTraining->id]) }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                        <i class="fa fa-eye"></i>
                    </a>
                    @endcan
                    @can('cooperativeTrainings edit')
                    <a href="{{ route('adminPanel.cooperativeTrainings.edit', [$cooperativeTraining->id]) . "?languages=en" }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-primary'>
                        <i class="fa fa-edit"></i>
                    </a>
                    @endcan
                    @can('cooperativeTrainings destroy')
                    {{-- {!! Form::button('<i class="fa fa-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-sm btn-shadow mx-1 btn-transparent-danger', 'onclick' => 'return confirm("'.__('crud.are_you_sure').'")']) !!} --}}
                    <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger" data-toggle="modal" data-target="#country-{{$cooperativeTraining->id}}-modal">
...
<!-- Modal -->
<div class="modal fade" id="country-{{$cooperativeTraining->id}}-modal" tabindex="-1" role="dialog" aria-labelledby="country-{{$cooperativeTraining->id}}"
    aria-hidden="true">
