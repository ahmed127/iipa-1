                    ...
                    {!! Form::open(['route' => 'adminPanel.partners.index', 'method'=>'GET']) !!}
                        <div class="row">
                            ...
                            <div class="form-group col-sm-4">
                                ...
                                <a href="{{ route('adminPanel.partners.index') }}" class="btn btn-light-danger font-weight-bold">@lang('crud.reset')</a>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    ...
    <tbody>
        ...
        <tr>
            <td>
                <label class="checkbox">
                    <input type="checkbox" class="check_inputs inputs-permmission control-input" value="{{ $partner->id }}" name="export_rows[]">
                    <span></span>
                </label>
            </td>
            <td>{{ $partner->logo }}</td>
            ...
            <td>
                ...
                <div class='btn btn-sm-group'>
                    @can('partners view')
                    <a href="{{ route('adminPanel.partners.show', [$partner->id]) }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                        <i class="fa fa-eye"></i>
                    </a>
                    @endcan
                    @can('partners edit')
                    <a href="{{ route('adminPanel.partners.edit', [$partner->id]) . "?languages=en" }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-primary'>
                        <i class="fa fa-edit"></i>
                    </a>
                    @endcan
                    @can('partners destroy')
                    {{-- {!! Form::button('<i class="fa fa-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-sm btn-shadow mx-1 btn-transparent-danger', 'onclick' => 'return confirm("'.__('crud.are_you_sure').'")']) !!} --}}
                    <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger" data-toggle="modal" data-target="#country-{{$partner->id}}-modal">
...
<!-- Modal -->
<div class="modal fade" id="country-{{$partner->id}}-modal" tabindex="-1" role="dialog" aria-labelledby="country-{{$partner->id}}"
    aria-hidden="true">
