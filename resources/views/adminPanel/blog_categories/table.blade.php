        ...
        <div class="col-lg-12 col-xl-12">
            <div class="row align-items-center">
                <div class="col-md-12 my-md-0">
                    {!! Form::open(['route' => 'adminPanel.blogCategories.index', 'method' => 'GET']) !!}
                    <div class="row">
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <a href="{{ route('adminPanel.blogCategories.index') }}"
                        ...
                        <div class="form-group col-sm-12">
                            @error('export_rows')
                                <h1 class="text-danger">
                                    @lang('lang.select_to_export', ['model' => __('models/blogCategories.plural')])
                                </h1>
                            @enderror
                        </div>
                    </div>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    <thead>
        <tr>
            ...
            <th>@lang('models/blogCategories.fields.photo')</th>
            <th>@lang('models/blogCategories.fields.title')</th>
            <th>@lang('crud.action')</th>
        </tr>
    </thead>
    <tbody>
        {!! Form::open(['route' => ['adminPanel.data.export', 'blogCategories'], 'id' => 'export-data']) !!}
        @foreach ($blogCategories as $blogCategory)
            <tr>
                <td>
                    <label class="checkbox">
                        <input type="checkbox" class="check_inputs inputs-permmission control-input"
                            value="{{ $blogCategory->id }}" name="export_rows[]">
                        <span></span>
                    </label>
                </td>
                <td><img src="{{ $blogCategory->photo_thumbnail_path }}" alt="{{ $blogCategory->title }}"
                        width="100"> </td>
                <td>{{ $blogCategory->title }}</td>
                <td>
                    {{-- {!! Form::open(['route' => ['adminPanel.blogCategories.destroy', $blogCategory->id], 'method' => 'delete']) !!} --}}
                    <div class='btn btn-sm-group'>
                        {{-- @can('blogCategories view')
                            <a href="{{ route('adminPanel.blogCategories.show', [$blogCategory->id]) }}"
                                class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                                <i class="fa fa-eye"></i>
                            </a>
                        @endcan --}}
                        @can('blogCategories edit')
                            <a href="{{ route('adminPanel.blogCategories.edit', [$blogCategory->id]) . '?languages=en' }}"
                        ...
                        @can('blogCategories destroy')
                            ...
                            <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger"
                                data-toggle="modal" data-target="#country-{{ $blogCategory->id }}-modal">
...
@can('blogCategories destroy')
    @foreach ($blogCategories as $blogCategory)
        <!-- Modal -->
        <div class="modal fade" id="country-{{ $blogCategory->id }}-modal" tabindex="-1" role="dialog"
            aria-labelledby="country-{{ $blogCategory->id }}" aria-hidden="true">
