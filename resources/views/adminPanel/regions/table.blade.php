        ...
        <div class="col-lg-12 col-xl-12">
            <div class="row align-items-center">
                <div class="col-md-12 my-md-0">
                    {!! Form::open(['route' => ['adminPanel.cities.regions.index', $city->id ], 'method' => 'GET']) !!}
                    <div class="row">
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <a href="{{ route('adminPanel.cities.regions.index', $city->id ) }}"
                        ...
                        <div class="form-group col-sm-12">
                            @error('export_rows')
                                <h1 class="text-danger">
                                    @lang('lang.select_to_export', ['model' => __('models/regions.plural')])
                                </h1>
                            @enderror
                        </div>
                    </div>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    ...
    <tbody>
        {!! Form::open(['route' => ['adminPanel.data.export', 'regions'], 'id' => 'export-data']) !!}
        @foreach ($regions as $region)
            <tr>
                <td>
                    <label class="checkbox">
                        <input type="checkbox" class="check_inputs inputs-permmission control-input"
                            value="{{ $region->id }}" name="export_rows[]">
                        <span></span>
                    </label>
                </td>
                <td>{{ $region->name }}</td>
                <td>
                    {{-- {!! Form::open(['route' => ['adminPanel.regions.destroy', $region->id], 'method' => 'delete']) !!} --}}
                    <div class='btn btn-sm-group'>
                        {{-- @can('regions view')
                            <a href="{{ route('adminPanel.regions.show', [$region->id]) }}"
                                class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                                <i class="fa fa-eye"></i>
                            </a>
                        @endcan --}}
                        @can('regions edit')
                            <a href="{{ route('adminPanel.regions.edit', [$region->id]) . '?languages=en' }}"
                        ...
                        @can('regions destroy')
                            ...
                            <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger"
                                data-toggle="modal" data-target="#country-{{ $region->id }}-modal">
...
@can('regions destroy')
    @foreach ($regions as $region)
        <!-- Modal -->
        <div class="modal fade" id="country-{{ $region->id }}-modal" tabindex="-1" role="dialog"
            aria-labelledby="country-{{ $region->id }}" aria-hidden="true">
