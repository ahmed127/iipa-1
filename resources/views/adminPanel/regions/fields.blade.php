...
<div class="tab-content mt-5" id="myTabContent">
    @foreach (config('langs') as $locale => $name)
        <div class="tab-pane fade {{ request('languages') == $locale ? 'show active' : '' }}" id="{{ $name }}"
            ...
            <div class="form-group col-sm-6">
                ...
                {!! Form::text($locale . '[name]', isset($region) ? $region->translate($locale)->name : '', ['class' => 'form-control', 'placeholder' => $name . ' name']) !!}
            </div>
        </div>
    @endforeach
    ...
    <div class="form-group col-sm-12">
        ...
        <a href="{{ route('adminPanel.cities.regions.index', $region->city_id ?? $city->id ) }}" class="btn btn-default">@lang('crud.cancel')</a>
    </div>
