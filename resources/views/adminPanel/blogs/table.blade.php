...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    <thead>
        <tr>
            ...
            <th>@lang('models/blogs.fields.blog_category_id')</th>
            <th>@lang('models/blogs.fields.photo_sm')</th>
            <th>@lang('models/blogs.fields.photo_cover')</th>
    ...
    <tbody>
        @foreach ($blogs as $blog)
            <tr>
                ...
                <td><img onError="this.onerror=null;this.src='{{ asset('uploads/images/original/default.png') }}';"
                        src="{{ $blog->photo_sm_original_path }}" alt="{{ $blog->title }}" width="150"></td>
                <td><img onError="this.onerror=null;this.src='{{ asset('uploads/images/original/default.png') }}';"
                        src="{{ $blog->photo_cover_original_path }}" alt="{{ $blog->title }}" width="150"></td>
                <td>{{ $blog->translateOrNew('en')->title }}</td>
                <td nowrap>
                    {!! Form::open(['route' => ['adminPanel.blogs.destroy', $blog->id], 'method' => 'delete']) !!}
                    <div class='btn btn-sm-group'>
                        @can('blogs view')
                            <a href="{{ route('adminPanel.blogs.show', [$blog->id]) }}"
                                class='btn btn-sm btn-shadow mx-1 btn-transparent-success'><i class="fa fa-eye"></i></a>
                        @endcan
                        @can('blogs edit')
                            <a href="{{ route('adminPanel.blogs.edit', [$blog->id]) . '?languages=en' }}"
