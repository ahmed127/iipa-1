        ...
        <div class="col-lg-12 col-xl-12">
            <div class="row align-items-center">
                <div class="col-md-12 my-md-0">
                    {!! Form::open(['route' => 'adminPanel.events.index', 'method'=>'GET']) !!}
                        <div class="row">
                            ...
                            <div class="form-group col-sm-4">
                                ...
                                <a href="{{ route('adminPanel.events.index') }}" class="btn btn-light-danger font-weight-bold">@lang('crud.reset')</a>
                            ...
                            <div class="form-group col-sm-12">
                                @error('export_rows')
                                    <h1 class="text-danger">
                                        @lang('lang.select_to_export', ['model' => __('models/events.plural')])
                                    </h1>
                                @enderror
                            </div>
                        </div>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    <thead>
        ...
        <th>@lang('models/events.fields.brief')</th>
    ...
    <tbody>
        {!! Form::open(['route' => ['adminPanel.data.export', 'events'], 'id' => 'export-data']) !!}
        @foreach($events as $event)
        <tr>
            <td>
                <label class="checkbox">
                    <input type="checkbox" class="check_inputs inputs-permmission control-input" value="{{ $event->id }}" name="export_rows[]">
                    <span></span>
                </label>
            </td>
            <td>{{ $event->title }}</td>
            ...
            <td>
                ...
                <div class='btn btn-sm-group'>
                    @can('events view')
                    <a href="{{ route('adminPanel.events.show', [$event->id]) }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                        <i class="fa fa-eye"></i>
                    </a>
                    @endcan
                    @can('events edit')
                    <a href="{{ route('adminPanel.events.edit', [$event->id]) . "?languages=en" }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-primary'>
                        <i class="fa fa-edit"></i>
                    </a>
                    @endcan
                    @can('events destroy')
                    {{-- {!! Form::button('<i class="fa fa-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-sm btn-shadow mx-1 btn-transparent-danger', 'onclick' => 'return confirm("'.__('crud.are_you_sure').'")']) !!} --}}
                    <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger" data-toggle="modal" data-target="#country-{{$event->id}}-modal">
...
<!-- Modal -->
<div class="modal fade" id="country-{{$event->id}}-modal" tabindex="-1" role="dialog" aria-labelledby="country-{{$event->id}}"
    aria-hidden="true">
