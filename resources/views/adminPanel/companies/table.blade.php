        ...
        <div class="col-lg-12 col-xl-12">
            <div class="row align-items-center">
                <div class="col-md-12 my-md-0">
                    {!! Form::open(['route' => 'adminPanel.companies.index', 'method' => 'GET']) !!}
                    <div class="row">
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            {!! Form::select('type', \App\Models\Company::types(), request('type') ?? null, [
                                'class' => 'form-control',
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <a href="{{ route('adminPanel.companies.index') }}"
                        ...
                        <div class="form-group col-sm-12">
                            @error('export_rows')
                                <h1 class="text-danger">
                                    @lang('lang.select_to_export', ['model' => __('models/companies.plural')])
                                </h1>
                            @enderror
                        </div>
                    </div>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    <thead>
        <tr>
            ...
            <th>@lang('models/companies.fields.website')</th>
            <th>@lang('models/companies.fields.location')</th>
            <th>@lang('models/companies.fields.type')</th>
            <th>@lang('crud.action')</th>
        </tr>
    </thead>
    <tbody>
        {!! Form::open(['route' => ['adminPanel.data.export', 'companies'], 'id' => 'export-data']) !!}
        @foreach ($companies as $company)
            <tr>
                <td>
                    <label class="checkbox">
                        <input type="checkbox" class="check_inputs inputs-permmission control-input"
                            value="{{ $company->id }}" name="export_rows[]">
                        <span></span>
                    </label>
                </td>
                <td>{{ $company->name }}</td>
                ...
                <td>
                    {{-- {!! Form::open(['route' => ['adminPanel.companies.destroy', $company->id], 'method' => 'delete']) !!} --}}
                    <div class='btn btn-sm-group'>
                        @can('companies edit')
                            <a href="{{ route('adminPanel.companies.edit', [$company->id]) . '?languages=en' }}"
                        ...
                        @can('companies destroy')
                            ...
                            <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger"
                                data-toggle="modal" data-target="#country-{{ $company->id }}-modal">
...
@can('companies destroy')
    @foreach ($companies as $company)
        <!-- Modal -->
        <div class="modal fade" id="country-{{ $company->id }}-modal" tabindex="-1" role="dialog"
            aria-labelledby="country-{{ $company->id }}" aria-hidden="true">
