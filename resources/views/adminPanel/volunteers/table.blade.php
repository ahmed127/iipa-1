                    ...
                    {!! Form::open(['route' => 'adminPanel.volunteers.index', 'method'=>'GET']) !!}
                        <div class="row">
                            ...
                            <div class="form-group col-sm-4">
                                ...
                                <a href="{{ route('adminPanel.volunteers.index') }}" class="btn btn-light-danger font-weight-bold">@lang('crud.reset')</a>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    ...
    <tbody>
        ...
        <tr>
            <td>
                <label class="checkbox">
                    <input type="checkbox" class="check_inputs inputs-permmission control-input" value="{{ $volunteer->id }}" name="export_rows[]">
                    <span></span>
                </label>
            </td>
            <td>{{ $volunteer->volunteer_type_id }}</td>
            <td>{{ $volunteer->full_name }}</td>
            <td>{{ $volunteer->id_no }}</td>
            <td>{{ $volunteer->email }}</td>
            <td>{{ $volunteer->country_code }}</td>
            <td>{{ $volunteer->phone }}</td>
            <td>{{ $volunteer->attachment_cv }}</td>
            <td>
                {{-- {!! Form::open(['route' => ['adminPanel.volunteers.destroy', $volunteer->id], 'method' => 'delete']) !!} --}}
                <div class='btn btn-sm-group'>
                    @can('volunteers view')
                    <a href="{{ route('adminPanel.volunteers.show', [$volunteer->id]) }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                        <i class="fa fa-eye"></i>
                    </a>
                    @endcan
                    @can('volunteers edit')
                    <a href="{{ route('adminPanel.volunteers.edit', [$volunteer->id]) . "?languages=en" }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-primary'>
                        <i class="fa fa-edit"></i>
                    </a>
                    @endcan
                    @can('volunteers destroy')
                    {{-- {!! Form::button('<i class="fa fa-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-sm btn-shadow mx-1 btn-transparent-danger', 'onclick' => 'return confirm("'.__('crud.are_you_sure').'")']) !!} --}}
                    <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger" data-toggle="modal" data-target="#country-{{$volunteer->id}}-modal">
...
<!-- Modal -->
<div class="modal fade" id="country-{{$volunteer->id}}-modal" tabindex="-1" role="dialog" aria-labelledby="country-{{$volunteer->id}}"
    aria-hidden="true">
