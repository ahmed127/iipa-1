                    ...
                    {!! Form::open(['route' => 'adminPanel.regulations.index', 'method'=>'GET']) !!}
                        <div class="row">
                            ...
                            <div class="form-group col-sm-4">
                                ...
                                <a href="{{ route('adminPanel.regulations.index') }}" class="btn btn-light-danger font-weight-bold">@lang('crud.reset')</a>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    <thead>
        ...
        <th>@lang('models/regulations.fields.attachment_pdf')</th>
    ...
    <tbody>
        ...
        <tr>
            <td>
                <label class="checkbox">
                    <input type="checkbox" class="check_inputs inputs-permmission control-input" value="{{ $regulation->id }}" name="export_rows[]">
                    <span></span>
                </label>
            </td>
            ...
            <td>{{ $regulation->photo }}</td>
            <td>{{ $regulation->attachment_pdf }}</td>
            <td>{{ $regulation->type }}</td>
            <td>
                {{-- {!! Form::open(['route' => ['adminPanel.regulations.destroy', $regulation->id], 'method' => 'delete']) !!} --}}
                <div class='btn btn-sm-group'>
                    @can('regulations view')
                    <a href="{{ route('adminPanel.regulations.show', [$regulation->id]) }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                        <i class="fa fa-eye"></i>
                    </a>
                    @endcan
                    @can('regulations edit')
                    <a href="{{ route('adminPanel.regulations.edit', [$regulation->id]) . "?languages=en" }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-primary'>
                        <i class="fa fa-edit"></i>
                    </a>
                    @endcan
                    @can('regulations destroy')
                    {{-- {!! Form::button('<i class="fa fa-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-sm btn-shadow mx-1 btn-transparent-danger', 'onclick' => 'return confirm("'.__('crud.are_you_sure').'")']) !!} --}}
                    <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger" data-toggle="modal" data-target="#country-{{$regulation->id}}-modal">
...
<!-- Modal -->
<div class="modal fade" id="country-{{$regulation->id}}-modal" tabindex="-1" role="dialog" aria-labelledby="country-{{$regulation->id}}"
    aria-hidden="true">
