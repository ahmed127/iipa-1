        ...
        <div class="col-lg-12 col-xl-12">
            <div class="row align-items-center">
                <div class="col-md-12 my-md-0">
                    {!! Form::open(['route' => 'adminPanel.packages.index', 'method' => 'GET']) !!}
                    <div class="row">
                        ...
                        <div class="form-group col-sm-4">
                            ...
                            <a href="{{ route('adminPanel.packages.index') }}"
                        ...
                        {{-- <div class="form-group col-sm-12">
                            @error('export_rows')
                                <h1 class="text-danger">
                                    @lang('lang.select_to_export', ['model' => __('models/packages.plural')])
                                </h1>
                            @enderror
                        </div> --}}
                    </div>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    ...
    <tbody>
        {!! Form::open(['route' => ['adminPanel.data.export', 'packages'], 'id' => 'export-data']) !!}
        @foreach ($packages as $package)
            <tr>
                <td>
                    <label class="checkbox">
                        <input type="checkbox" class="check_inputs inputs-permmission control-input"
                            value="{{ $package->id }}" name="export_rows[]">
                        <span></span>
                    </label>
                </td>
                <td>{{ $package->name }}</td>
                ...
                <td>
                    ...
                    <div class='btn btn-sm-group'>
                        @can('packages view')
                            <a href="{{ route('adminPanel.packages.show', [$package->id]) }}"
                                class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                                <i class="fa fa-eye"></i>
                            </a>
                        @endcan
                        @can('packages edit')
                            <a href="{{ route('adminPanel.packages.edit', [$package->id]) . '?languages=en' }}"
                        ...
                        @can('packages destroy')
                            ...
                            <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger"
                                data-toggle="modal" data-target="#country-{{ $package->id }}-modal">
...
@can('packages destroy')
    @foreach ($packages as $package)
        <!-- Modal -->
        <div class="modal fade" id="country-{{ $package->id }}-modal" tabindex="-1" role="dialog"
            aria-labelledby="country-{{ $package->id }}" aria-hidden="true">
