                    ...
                    {!! Form::open(['route' => 'adminPanel.outreaches.index', 'method'=>'GET']) !!}
                        <div class="row">
                            ...
                            <div class="form-group col-sm-4">
                                ...
                                <a href="{{ route('adminPanel.outreaches.index') }}" class="btn btn-light-danger font-weight-bold">@lang('crud.reset')</a>
...
<table class="table table-bordered table-hover" id="kt_datatableasd">
    <thead>
        ...
        <th>@lang('models/outreaches.fields.description')</th>
        <th>@lang('models/outreaches.fields.photo')</th>
        <th>@lang('models/outreaches.fields.attachment_pdf')</th>
    ...
    <tbody>
        ...
        <tr>
            <td>
                <label class="checkbox">
                    <input type="checkbox" class="check_inputs inputs-permmission control-input" value="{{ $outreach->id }}" name="export_rows[]">
                    <span></span>
                </label>
            </td>
            <td>{{ $outreach->title }}</td>
            <td>{{ $outreach->brief }}</td>
            <td>{{ $outreach->description }}</td>
            <td>{{ $outreach->photo }}</td>
            <td>{{ $outreach->attachment_pdf }}</td>
            <td>{{ $outreach->type }}</td>
            <td>
                {{-- {!! Form::open(['route' => ['adminPanel.outreaches.destroy', $outreach->id], 'method' => 'delete']) !!} --}}
                <div class='btn btn-sm-group'>
                    @can('outreaches view')
                    <a href="{{ route('adminPanel.outreaches.show', [$outreach->id]) }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-success'>
                        <i class="fa fa-eye"></i>
                    </a>
                    @endcan
                    @can('outreaches edit')
                    <a href="{{ route('adminPanel.outreaches.edit', [$outreach->id]) . "?languages=en" }}" class='btn btn-sm btn-shadow mx-1 btn-transparent-primary'>
                        <i class="fa fa-edit"></i>
                    </a>
                    @endcan
                    @can('outreaches destroy')
                    {{-- {!! Form::button('<i class="fa fa-trash"></i>', ['type' => 'submit', 'class' => 'btn btn-sm btn-shadow mx-1 btn-transparent-danger', 'onclick' => 'return confirm("'.__('crud.are_you_sure').'")']) !!} --}}
                    <button type="button" class="btn btn-sm btn-shadow mx-1 btn-transparent-danger" data-toggle="modal" data-target="#country-{{$outreach->id}}-modal">
...
<!-- Modal -->
<div class="modal fade" id="country-{{$outreach->id}}-modal" tabindex="-1" role="dialog" aria-labelledby="country-{{$outreach->id}}"
    aria-hidden="true">
