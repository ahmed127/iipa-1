                    ...
                    <div class="row gx-0 p-3">
                        ...
                        <div class="row my-3 p-3">
                            @switch($follow_request->department)
                                @case(1)
                                    @include('adminPanel.consultings.show_fields', [
                                        'consulting' => $follow_request->forable,
                                    ])
                                @break
                                ...
                                @case(3)
                                    @include('adminPanel.volunteers.show_fields', [
                                        'volunteer' => $follow_request->forable,
                                    ])
                                @break

                                @case(4)
                                    @include('adminPanel.cooperative_trainings.show_fields', [
                                        'cooperativeTraining' => $follow_request->forable,
                                    ])
                                @break

                                @case(5)
                                    @include('adminPanel.individual_trainings.show_fields', [
                                        'individualTraining' => $follow_request->forable,
                                    ])
                                @break

                                @case(6)
                                    @include('adminPanel.contacts.show_fields', [
                                        'contact' => $follow_request->forable,
                                    ])
                                @break

                                @case(7)
                                    @include('adminPanel.recruitments.show_fields', [
                                        'recruitment' => $follow_request->forable,
                                    ])
                                @break
